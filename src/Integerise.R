...
int_expand <-
  apply(weights, 2, function(x)
    int_expand_vector(int_trs(x))) # generate integerised result

ints <- unlist(int_expand)

if (!identical(lengths(int_expand), round(colSums(weights)) %>% as.integer()))
  myStop("Expansion error.")

ints_df <-
  data.frame(id = ints, zone = rep(1:nrow(cons), round(colSums(weights))))

ind$id <- 1:nrow(ind) # assign each individual an id

# Create spatial microdata, by joining the ids with associated attributes
ints_df <-
  inner_join(ints_df, ind %>% select(pidp, id)) # commented out to increase run-times
